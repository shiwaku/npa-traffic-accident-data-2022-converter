...
def convert():
    ...
    dict_keisatusyo = load_code_dict(
        ROOT / 'code' / '3_koudohyou_keisatusyotoukoudo_2022.csv', [0, 1], 3)
    ...
    # 本票データ読み込み
    data = pd.read_csv(ROOT / 'honhyo_2022_to-degree.csv', dtype=object).values.tolist()
    print(f'{len(data)}件')
    ...
    with open(ROOT / 'honhyo_2022_convert.csv', 'w', encoding='UTF-8') as f:
